...
path1 = os.path.join(os.path.abspath(__file__+"/.."), "space")
...
enemy_img1 = os.path.join(path1, "ufo2.png")
enemy_img1 = image.load(enemy_img1)

bullet_img = os.path.join(path1, "bullet.png")
bullet_img = image.load(bullet_img)

bullet_img2 = os.path.join(path1, "bullet2.png")
bullet_img2 = image.load(bullet_img2)
...
class GameSprite(sprite.Sprite):
    def __init__(self, player_image, player_image2, player_x, player_y, player_speed, width, height):
        super().__init__()
        self.width = width
        self.height = height
        self.player_image2 = transform.scale(player_image2, (width, height))
    ...
    def reset(self):
        ...
        if anim <= 500:
            ...
            window.blit(self.player_image2, (self.rect.x, self.rect.y))
            if anim >= 1000:
                anim = 0
        anim += 1

class Player(GameSprite):
    def __init__(self, player_image, player_image_left, player_image_right,  player_image2, player_x, player_y, player_speed, width, height):
        super().__init__(player_image, player_image2, player_x, player_y, player_speed, width, height)
        self.player_image_left = transform.scale(player_image_left, (width, height))
        self.player_image_right = transform.scale(player_image_right, (width, height))
    def update(self):
        ...
        if (key.get_pressed()[K_d] or key.get_pressed()[K_RIGHT]) and self.rect.x < 640:
            ...
            '''if self.le_ra >= 2:
                ...
                self.le_ra = 1'''
    def fire(self):
        bul = Bullet(bullet_img, bullet_img2, player.rect.centerx - 2, player.rect.top, 6, 10, 20)
...
def numbers(text1, shirina, x_cord, y_cord, color, txt):                              
    f1 = font.Font(None, shirina)                                             
    text1 = f1.render(txt + str(text1), True, (color))                           
    window.blit(text1, (x_cord, y_cord))                               
                                                                                                                               
player = Player(player_image, player_image, player_image, player_image, 300, 400, 8, 64, 80)                     
player.bullets = []                                                             
                                                        
#Список врагов                                                    
enemies = []                                                   
for i in range(5):                                                     
    enemy = Enemy(enemy_img, enemy_img1, randint(0, 600), -100, randint(1,5), 64, 128)
...
def reset_game(enemies):
    ...
    for gr1 in enemies:
        ...
        gr1.rect.y = -60
    print(enemies, 'BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB')
...
while game:
    ...
    if move == 0:
        ...
        numbers('нажмите на стрелку вверх!', 35, 40, 300, (180, 0, 0), 'Чтобы начать игру заново, ')
